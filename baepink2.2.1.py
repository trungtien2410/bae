...
class Worker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            # Create a DataFrame for the final grouped IDs one column for ten_or_more and another for six_to_nine and six_to_nine does not include ids in ten_or_more
            if final_grouped_ids_ten:
                df_output_ids = pd.DataFrame(list(final_grouped_ids_ten), columns=['ID >=10'])
            if final_grouped_ids_six_to_nine:
                df_six_to_nine = pd.DataFrame(list(final_grouped_ids_six_to_nine - final_grouped_ids_ten), columns=['ID 6-9'])
...
class Worker12(QtCore.QThread):
    """
    Lớp con của QThread để thực hiện việc nhóm dữ liệu Same phone and district NUV
    Phát tín hiệu để cập nhật tiến độ, thông báo nhật ký và trạng thái hoàn thành.
    """
    progress = QtCore.pyqtSignal(int)
    log = QtCore.pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            required_columns = ['recipient_phone_', 'buyer_id', 'buyer_shipping_address_district']
            if not all(col in self.df.columns for col in required_columns):
                missing_cols = [col for col in required_columns if col not in self.df.columns]
                self.log.emit(f"❌ Lỗi: File Excel thiếu các cột bắt buộc: {', '.join(missing_cols)}")
                self.finished.emit(None)
            ...
            # ******************************************************
            # 5. Core Logic Adjustment: Group by Phone and District, count UNIQUE Buyer IDs
            # ******************************************************
            grouped_summary = df_processed.groupby(
                ['recipient_phone_', 'buyer_shipping_address_district']
            )['buyer_id'].nunique().reset_index(name='unique_buyer_ids_count')

            # ******************************************************
            # 6. Filter Groups
            # Get the groups (phone, district) that meet the criteria
            # ******************************************************
            
            # Groups with 10 or more unique buyer_id's
            filtered_groups_ten = grouped_summary[grouped_summary['unique_buyer_ids_count'] >= 10]
            
            # Groups with 6 to 9 unique buyer_id's
            filtered_groups_six_to_nine = grouped_summary[
                (grouped_summary['unique_buyer_ids_count'] >= 6) & 
                (grouped_summary['unique_buyer_ids_count'] < 10)
            ]

            # ******************************************************
            # 7. Optimized ID Retrieval using Merging 🚀
            # ******************************************************
            final_grouped_ids_ten = set()
            ids_in_six_to_nine_groups = set()

            # --- IDs >= 10 ---
            if not filtered_groups_ten.empty:
                # Merge original df with the filtered groups summary on the grouping keys
                merged_ten = pd.merge(
                    df_processed,
                    filtered_groups_ten[['recipient_phone_', 'buyer_shipping_address_district']],
                    on=['recipient_phone_', 'buyer_shipping_address_district'],
                    how='inner'
                )
                final_grouped_ids_ten.update(merged_ten['buyer_id'].unique())
            
            # --- IDs 6-9 ---
            if not filtered_groups_six_to_nine.empty:
                # Merge original df with the 6-9 filtered groups summary
                merged_six_to_nine = pd.merge(
                    df_processed,
                    filtered_groups_six_to_nine[['recipient_phone_', 'buyer_shipping_address_district']],
                    on=['recipient_phone_', 'buyer_shipping_address_district'],
                    how='inner'
                )
                ids_in_six_to_nine_groups.update(merged_six_to_nine['buyer_id'].unique())

            # Exclude IDs already found in the 'ten' group
            final_grouped_ids_six_to_nine = ids_in_six_to_nine_groups - final_grouped_ids_ten
            ...
            if final_grouped_ids_ten:
                output_columns.append(pd.Series(list(final_grouped_ids_ten), name='ID >=10'))
            
            if final_grouped_ids_six_to_nine:
                output_columns.append(pd.Series(list(final_grouped_ids_six_to_nine), name='ID 6-9'))
            ...
            if output_columns:
                ...
                # Save to Excel
                df_output_ids.to_excel(self.output_file_path, index=False, engine='openpyxl')
                self.log.emit(f"✅ Hoàn thành! Đã lưu {len(final_grouped_ids_ten) + len(final_grouped_ids_six_to_nine)} ID vào file.")
            else:
                self.log.emit("ℹ️ Không tìm thấy ID nào để nhóm theo tiêu chí (recipient_phone_, buyer_shipping_address_district >= 6 ID).")
...
class Worker13(QtCore.QThread):
    """
    Lớp con của QThread để thực hiện việc nhóm dữ liệu Same name, district, city and state NUV
    Phát tín hiệu để cập nhật tiến độ, thông báo nhật ký và trạng thái hoàn thành.
    """
    progress = QtCore.pyqtSignal(int)
    log = QtCore.pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            # 2. Drop Columns
            drop_column = ["grass_hour","create_time","order_id","item_name","seller_id","shop_name","status_b","buyer_user_name","buyer_email","recipient_phone_","buyer_shipping_address","address_modified_time_latest","sz_device","ip_checkout","gmv_vnd","pv_promotion_cap","pv_promotion_name","pv_voucher_code","pv_rebate_by_shopee_vnd","is_nuv","sv_promotion_id","sv_voucher_code","coin_earn","coin_used_cash_amt","fsv_voucher_code","is_fsv_nuv","origin_shipping_fee_vnd","item_rebate_vnd","item_id","is_buyer_legit","is_seller_cb_seller","is_seller_official_shop","is_seller_preferred_seller","order_sn","buyer_cancel_reason"]
            self.df = self.df.drop(columns=drop_column, errors='ignore')
            
            # 3. Validate required columns
            required_columns = ['recipient_name', 'buyer_id', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state']
            if not all(col in self.df.columns for col in required_columns):
                missing_cols = [col for col in required_columns if col not in self.df.columns]
                self.log.emit(f"❌ Lỗi: File Excel thiếu các cột bắt buộc: {', '.join(missing_cols)}")
                self.finished.emit(None)
            ...
            # ******************************************************
            grouped_summary = df_processed.groupby(
                ['recipient_name', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state']
            )['buyer_id'].nunique().reset_index(name='unique_buyer_ids_count')

            # ******************************************************
            # 6. Filter Groups
            # Get the groups (name, district, city, state) that meet the criteria
            # ******************************************************
            ...
            if not filtered_groups_ten.empty:
                ...
                merged_ten = pd.merge(
                    df_processed,
                    filtered_groups_ten[['recipient_name', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state']],
                    on=['recipient_name', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state'],
                    how='inner'
                )
                final_grouped_ids_ten.update(merged_ten['buyer_id'].unique())
            ...
            if not filtered_groups_six_to_nine.empty:
                ...
                merged_six_to_nine = pd.merge(
                    df_processed,
                    filtered_groups_six_to_nine[['recipient_name', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state']],
                    on=['recipient_name', 'buyer_shipping_address_district', 'buyer_shipping_address_city', 'buyer_shipping_address_state'],
                    how='inner'
                )
                ids_in_six_to_nine_groups.update(merged_six_to_nine['buyer_id'].unique())
            ...
            if output_columns:
                ...
                self.log.emit("ℹ️ Không tìm thấy ID nào để nhóm theo tiêu chí (recipient_name, buyer_shipping_address_district, buyer_shipping_address_city, buyer_shipping_address_state >= 6 ID).")
...
class Worker14(QtCore.QThread):
    """
    Lớp con của QThread để thực hiện việc nhóm dữ liệu Same IP +  District + City + State NUV and Create time within 01 hour Report
    Phát tín hiệu để cập nhật tiến độ, thông báo nhật ký và trạng thái hoàn thành.
    """
    progress = QtCore.pyqtSignal(int)
    log = QtCore.pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            GROUPING_KEYS = ['ip_checkout', 'buyer_shipping_address_district', 
                             'buyer_shipping_address_city', 'buyer_shipping_address_state']
            ...
            for key, group in df_processed.groupby(GROUPING_KEYS, sort=False):
                ...
                while i < len(group_list):
                    ...
                    if count >= 10:
                        final_grouped_ids_ten.update(unique_ids_in_cluster)
                        # The next window MUST start after this cluster ends (index j)
                        i = j
                    elif count >= 6:
                        # Only add to the temporary 6-9 set
                        ids_in_six_to_nine_groups.update(unique_ids_in_cluster)
                        # The next window MUST start after this cluster ends (index j)
                        i = j
                    else:
            ...
            # 5. Exclude IDs already found in the 'ten' group
            final_grouped_ids_six_to_nine = ids_in_six_to_nine_groups - final_grouped_ids_ten

            self.log.emit("ℹ️ Đang lưu kết quả...")
            
            # 6. Create Output DataFrame (same as before)
            output_columns = []
            ...
            if output_columns:
                ...
                self.log.emit("ℹ️ Không tìm thấy ID nào để nhóm theo tiêu chí (Same IP + District + City + State, >= 6 ID trong 1 giờ).")
...
class Worker15(QtCore.QThread):
    """
    Lớp con của QThread để thực hiện việc nhóm dữ liệu Same IP and registration_time and create_time Nuv
    Phát tín hiệu để cập nhật tiến độ, thông báo nhật ký và trạng thái hoàn thành.
    """
    progress = QtCore.pyqtSignal(int)
    log = QtCore.pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            # 2. Column Dropping and Validation
            drop_column = ["grass_hour", "order_id", "item_name", "seller_id", "shop_name", "status_b", 
                            "buyer_user_name", "buyer_email", "recipient_phone_", "recipient_name", 
                            "buyer_shipping_address", "buyer_shipping_address_district", 
                            "buyer_shipping_address_city", "buyer_shipping_address_state", 
            ...
            # Sort by IP and then create_time for sequential processing within the group
            df_processed.sort_values(by=['ip_checkout', 'create_time'], inplace=True)
            
            # 4. Core Logic: Fixed Start Time Grouping with Dual Time Check
            final_grouped_ids_ten = set()
            ids_in_six_to_nine_groups = set()
            
            total_unique_ips = len(df_processed['ip_checkout'].unique())
            processed_ips = 0
            TIME_WINDOW = timedelta(hours=1)

            # Iterate through each unique ip_checkout group
            for ip_checkout_val, group in df_processed.groupby('ip_checkout', sort=False):
                processed_ips += 1
                self.progress.emit(int((processed_ips / total_unique_ips) * 100))
                
                records_for_ip = group.to_dict('records')
                
                # Use a while loop to control the starting point 'i'
                i = 0
                while i < len(records_for_ip):
                    start_record = records_for_ip[i]
                    start_create_time = start_record['create_time']
                    start_reg_time = start_record['registration_time']
                    
                    # Find the index of the last record within BOTH 1-hour windows
                    j = i + 1
                    while j < len(records_for_ip):
                        next_record = records_for_ip[j]
                        
                        # Check 1: create_time must be within 1 hour of start_create_time
                        create_time_diff = next_record['create_time'] - start_create_time
                        
                        # Check 2: registration_time must be within 1 hour of start_reg_time
                        # Note: We use absolute difference here because reg_time might be earlier or later,
                        # but often we want to see if they are "close" to the first reg_time.
                        # If you require next_record['registration_time'] >= start_reg_time, remove abs().
                        registration_time_diff = next_record['registration_time'] - start_reg_time
                        
                        # The record must satisfy BOTH time window conditions
                        if create_time_diff <= TIME_WINDOW and registration_time_diff <= TIME_WINDOW:
                            j += 1
                        else:
                            break
                    
                    # The cluster includes records from index i up to (but not including) j
                    current_cluster = records_for_ip[i:j]
                    
                    # Get the unique buyer IDs in this fixed 1-hour cluster
                    unique_ids_in_cluster = set(record['buyer_id'] for record in current_cluster)
                    count = len(unique_ids_in_cluster)

                    # 5. Collect IDs and Advance Index (Non-overlapping logic)
                    if count >= 10:
                        final_grouped_ids_ten.update(unique_ids_in_cluster)
                        # Advance 'i' to the end of the current valid cluster
                        i = j
                    elif count >= 6:
                        ids_in_six_to_nine_groups.update(unique_ids_in_cluster)
                        # Advance 'i' to the end of the current valid cluster
                        ...
                        # If the cluster is too small, advance 'i' by one to check the next start time
                        i += 1 

            # 6. Exclude IDs and Final Output Preparation
            final_grouped_ids_six_to_nine = ids_in_six_to_nine_groups - final_grouped_ids_ten
            ...
            if output_columns:
                ...
                self.log.emit("ℹ️ Không tìm thấy ID nào để nhóm theo tiêu chí (Same IP, >= 6 ID trong 1 giờ cho cả create_time và registration_time).")
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.dark_mode_checkbox.setStyleSheet("QCheckBox { color: #0281b0; }") # Initial color for light mode
        ...
        self.label_mnv2 = QtWidgets.QLabel("Hãy chọn thư mục đích:")
        ...
        self.destination_folder.setFont(QtGui.QFont("Segoe UI", 11))
        ...
        btn_layout_column1 = QtWidgets.QVBoxLayout()
        ...
        btn_layout_column1.addWidget(self.clear_btn)
        btn_layout_column1.addWidget(self.fsv_btn)
        btn_layout_column1.addWidget(self.ip_create_time_btn)
        btn_layout_column1.addWidget(self.same_prm_phone_district_btn)
        ...
        checkbox_layout_column1.addWidget(self.same_prm_phone_district_btn_checkbox)
        checkbox_layout_column1.addWidget(self.same_ip_reg_create_time_btn_checkbox)

        
        btn_layout_column2 = QtWidgets.QVBoxLayout()
        ...
        btn_layout_column2.addWidget(self.same_recipient_phone_btn)
        btn_layout_column2.addWidget(self.same_order_value_check_out_and_similar_address_btn)
        btn_layout_column2.addWidget(self.tolerant_address_btn)
        btn_layout_column2.addWidget(self.rsl_btn)
        ...
        checkbox_layout_column2.addWidget(self.same_order_value_check_out_and_similar_address_btn_checkbox)
        checkbox_layout_column2.addWidget(self.tolerant_address_btn_checkbox)

        btn_layout_column3 = QtWidgets.QVBoxLayout()
        self.create_btn = QtWidgets.QPushButton("N3 Report")
        self.create_btn.setStyleSheet("color: black;") # Highlight primary action and bold text
        self.create_btn.clicked.connect(self.generate_report)
        self.same_phone_and_district_btn = QtWidgets.QPushButton("Same Phone and District Report")
        self.same_phone_and_district_btn.clicked.connect(self.same_phone_district_report)
        self.same_phone_and_district_btn.setStyleSheet("color: black;")
        self.same_name_district_city_state_btn = QtWidgets.QPushButton("Same Name + District + City + State Report")
        self.same_name_district_city_state_btn.clicked.connect(self.same_name_district_city_state_report)
        self.same_name_district_city_state_btn.setStyleSheet("color: black;")
        self.same_ip_create_time_district_city_state_btn = QtWidgets.QPushButton("Same IP and Create Time + District + City + State Report")
        self.same_ip_create_time_district_city_state_btn.clicked.connect(self.same_ip_create_time_district_city_state_report)
        self.same_ip_create_time_district_city_state_btn.setStyleSheet("color: black;")
        self.same_ip_create_reg_time_nuv_btn = QtWidgets.QPushButton("Same IP and Create + RegTime Nuv Report")
        self.same_ip_create_reg_time_nuv_btn.clicked.connect(self.same_ip_create_time_report_Nuv)
        self.same_ip_create_reg_time_nuv_btn.setStyleSheet("color: black;")
        btn_layout_column3.addWidget(self.create_btn)
        btn_layout_column3.addWidget(self.same_phone_and_district_btn)
        btn_layout_column3.addWidget(self.same_name_district_city_state_btn)
        btn_layout_column3.addWidget(self.same_ip_create_time_district_city_state_btn)
        ...
        checkbox_layout_column3.addWidget(self.ip_create_time_district_city_state_btn_checkbox)
        checkbox_layout_column3.addWidget(self.same_ip_create_reg_time_nuv_btn_checkbox)
        ...
        button_layout.addLayout(btn_layout_column1)
        button_layout.addLayout(checkbox_layout_column1)
        # button_layout.addSpacing(40)
        button_layout.addLayout(btn_layout_column2)
        button_layout.addLayout(checkbox_layout_column2)
        button_layout.addLayout(btn_layout_column3)
        button_layout.addLayout(checkbox_layout_column3)
        tab1_layout.addLayout(label_mnv_layout)
        tab1_layout.addLayout(label_mnv_layout2)
    ...
    def tolerant_address(self):
        ...
        self.thread.finished.connect(self.on_report_finished)
        self.thread.start()
    def same_phone_district_report(self):
        """
        Tạo báo cáo same phone và district.
        """
        input_file_path = self.mnv.text()
        if not input_file_path:
            QtWidgets.QMessageBox.warning(None, "Lỗi", "Vui lòng chọn file Excel gốc.")
        ...
        output_file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Lưu File Same", "same_phone_district.xlsx", "Excel Files (*.xlsx)")
        if not output_file_path:
            self.log_output.append("❌ Đã hủy lưu file.")
            self._set_buttons_enabled(True)
            return
    ...
    def same_name_district_city_state_report(self):
        ...
        self.thread.finished.connect(self.on_report_finished)
        self.thread.start()
    def same_ip_create_time_district_city_state_report(self):
        """
        Tạo báo cáo same IP và create time và district, city, state.
        """
        input_file_path = self.mnv.text()
        if not input_file_path:
            QtWidgets.QMessageBox.warning(None, "Lỗi", "Vui lòng chọn file Excel gốc.")
        ...
        output_file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Lưu File Same", "same_ip_create_time_district_city_state.xlsx", "Excel Files (*.xlsx)")
        if not output_file_path:
            self.log_output.append("❌ Đã hủy lưu file.")
            self._set_buttons_enabled(True)
            return
        ...
        self.thread.finished.connect(self.on_report_finished)
        self.thread.start()
    def same_ip_create_time_report_Nuv(self):
        """
        Tạo báo cáo same IP và create time Nuv.
        """
        input_file_path = self.mnv.text()
        if not input_file_path:
            QtWidgets.QMessageBox.warning(None, "Lỗi", "Vui lòng chọn file Excel gốc.")
        ...
        output_file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Lưu File Same", "same_ip_create_time_Nuv.xlsx", "Excel Files (*.xlsx)")
        if not output_file_path:
            self.log_output.append("❌ Đã hủy lưu file.")
            self._set_buttons_enabled(True)
            return
