...
class Worker2(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            grouped_df = self.df.groupby(['recipient_phone_', 'pv_promotion_id'])['buyer_id'].nunique().reset_index(name='unique_buyer_ids_count')
...
class Worker3(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            # Group by recipient_phone_ and pv_promotion_id
            # Then, for each group, find the number of unique buyer_id's
            grouped_df = self.df.groupby(['recipient_phone_', 'fsv_voucher_code'])['buyer_id'].nunique().reset_index(name='unique_buyer_ids_count')
